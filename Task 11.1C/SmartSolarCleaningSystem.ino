...
const int waterSensorPin = 3;      // Water level sensor (digital output)
...
void loop() {
  int isDusty = digitalRead(dustSensorPin);     // LOW = dusty
  int waterOK = digitalRead(waterSensorPin);    // HIGH = water present

  // Print time and sensor status
  printSimulatedClock();
  Serial.print(" | Dust: ");
  Serial.print(isDusty == LOW ? "DUSTY" : "CLEAN");
  Serial.print(" | Water: ");
  Serial.println(waterOK == HIGH ? "OK" : "LOW");

  if (isDusty == LOW && waterOK == HIGH) {
